...
export const PillToggle: React.FC<{ label: string; on: boolean; onClick: () => void; activeColor?: string; disabled?: boolean }> = ({ label, on, onClick, activeColor = '#1f6feb', disabled = false }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    style={{
      padding: '3px 8px', fontSize: 10, letterSpacing: 0.5,
      borderRadius: 4, cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
      background: on ? `${activeColor}33` : '#1a1a1a',
      color: disabled ? '#3f3f3f' : on ? '#fff' : '#777',

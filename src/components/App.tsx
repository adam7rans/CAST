...
import { useClipDiscovery } from '../hooks/useClipDiscovery';
import { useRefSync, useParamPush } from '../hooks/useParamSync';
import { AppLayout } from './app/AppLayout';
import { useAppDerivedState, type FullChunkRange } from './app/useAppDerivedState';
...
export const App: React.FC = () => {
  ...
  const [compositionMode, setCompositionMode] = useState<CompositionMode>('video');
  const [visualizer, setVisualizer] = useState<AudioVisualizerParams>(DEFAULT_AUDIO_VISUALIZER);
  ...
  const handlePropagateCaptions = useCallback(() => {
    setCaptionStyleByGuide((prev) => seedGuideMap(prev[captionSlotKeyRef.current] ?? DEFAULT_CAPTION_STYLE));
    setCaptionShaderByGuide((prev) => seedGuideMap(prev[captionSlotKeyRef.current] ?? DEFAULT_CAPTION_SHADER));
    addToast('Caption settings propagated to all crops', 'success');
  }, []);
  ...
  const [fullChunkOverrides, setFullChunkOverrides] = useState<Record<string, FullChunkRange>>({});
  ...
  useEffect(() => {
    if (mediaElRef.current) mediaElRef.current.playbackRate = playbackRate;
    if (musicElRef.current) musicElRef.current.playbackRate = playbackRate;
  }, [playbackRate, videoInfo, audioInfo]);
  ...
  const derived = useAppDerivedState({
    transcript, captionClipEdits, editorMode, microTimelines, selectedClipId, selectedFullSegmentId, setSelectedFullSegmentId, fullChunkOverrides,
  ...
  const visualizerRendererRef = useRef<AudioVisualizerRenderer | null>(null);
  ...
  useRefSync({ bgLayerOnRef, videoLayerOnRef, audioReactivityRef, visualizerRef, compositionModeRef, musicRef, playingRef, playheadRef, activeExportParamsRef, timelineDurationRef, selectedClipRef, activeProjectIdRef }, { bgLayerOn, videoLayerOn, audioReactivity, visualizer, compositionMode, music, playing, playheadSecond, activeExportParams, timelineDuration, selectedClip: selectedTimelineSegment, activeProjectId });
  useRenderLoop({ previewWrapRef, bgCanvasRef, videoCanvasRef, visualizerCanvasRef, bgRendererRef, videoRendererRef, visualizerRendererRef, audioSourceRef, musicPlayerRef, videoBlobUrlRef, audioBlobUrlRef, rafRef, startRef, exportingRef, lastBandsRef, speechRmsRef, musicDuckGainRef, limiterReductionRef, bgLayerOnRef, videoLayerOnRef, audioReactivityRef, visualizerRef, compositionModeRef, musicRef, playheadRef, activeExportParamsRef, timelineDurationRef }, bg, bgDither, vid, setPreviewSize);
  ...
  useEffect(() => {
    if (audioMode) setCompositionMode('audio');
    else if (videoInfo) setCompositionMode('video');
  }, [audioMode, videoInfo]);
  ...
  const { currentPresetSettings, applyPresetSettings } = usePresetSettings({
    state: { bg, bgDither, vid, audioReactivity, music, limiter, captionMode, captionStyle, captionShader, captionStyleByGuide, captionShaderByGuide, bgLayerOn, videoLayerOn, captionsLayerOn, musicLayerOn, bgOffMode, bgOffColor, activeGuide, cropToGuide },
  ...
  const { handleAppendClipCandidates, handleClipRangeChange, handleAddClipStart, handleAddClipEnd, handleDeleteClip, handleRenameClip, handleToggleOutro } = useClipHandlers({
  ...
  // Full mode: chunks are auto-tiled, so trims are stored as per-chunk overrides (clamped to the tile in derived state).
  const handleFullChunkRangeChange = (id: string, s: number, e: number) =>
    setFullChunkOverrides((prev) => ({ ...prev, [id]: { startSecond: s, endSecond: e } }));
  const activeProject = projects.find((p) => p.id === activeProjectId);
  const discovery = useClipDiscovery(activeProjectId, transcript);
  const clipsWorkspace = { discovery, projectId: activeProjectId, transcript, mediaDuration: videoInfo?.duration ?? audioInfo?.duration ?? 0,
    onAppend: (candidates: Parameters<typeof handleAppendClipCandidates>[0], duration: number) => { handleAppendClipCandidates(candidates, duration); setEditorMode('clips'); },
    onSeek: (second: number) => { setEditorMode('clips'); setSelectedClipId(null); playingInClipRef.current = false; controls.handleSeekPlayhead(second); },
  };
  const exportLayerSummary = [bgLayerOn ? 'background' : null, compositionMode === 'audio' && visualizer.enabled ? 'visualizer' : videoLayerOn ? 'video' : null, captionsLayerOn ? 'captions' : null].filter(Boolean).join(' + ') || 'none';
